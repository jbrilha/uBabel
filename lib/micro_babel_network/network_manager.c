...
static bool try_connect_wifi(const wifi_credentials_t* creds) {
    int auth = strlen(creds->pass) ? CYW43_AUTH_WPA2_AES_PSK : CYW43_AUTH_OPEN;
    printf("Trying SSID: %s (%s)\n", creds->ssid, strlen(creds->pass) ? "secured" : "open");

    return cyw43_arch_wifi_connect_timeout_ms(creds->ssid,
                                              strlen(creds->pass) ? creds->pass : NULL,
                                              auth, 10000) == 0;
}
...
void scan_for_open_networks() {
    ...
    if( scan_in_progress && cyw43_wifi_scan_active(&cyw43_state) && !scan_has_completed) {
        printf("Scan still in progress...\n");   
        cyw43_arch_poll(); 
        return;
    }
...
bool check_connectivity_via_ping_gateway() {
    ...
    int sent = lwip_sendto(s, &hdr, sizeof(hdr), 0,
    ...
    if (recv_len >= (int)(sizeof(struct ip_hdr) + sizeof(struct icmp_echo_hdr))) {
        ...
        struct icmp_echo_hdr *icmphdr = (struct icmp_echo_hdr *)(recv_buf + (IPH_HL(iphdr) * 4));
...
void network_manager_task(void* params) {
    ...
    while(true) {
        ...
        if(!connected) {
            ...
            if(!scan_in_progress) {
                ...
                for (int i = 0; i < cfg.wifi_list_size && !connected; ++i) {
                    ...
                    if (connected) {
                        ...
                        ip4addr_ntoa_r(&ip, evt->ip, 15); // Convert IP to string

                        event_t* event = create_event(EVENT_TYPE_NOTIFICATION, EVENT_SUBTYPE_NETWORK_UP, evt, sizeof(network_event_t*));
                        ...
                        printf("[EVENT] Connected to %s | IP: %s\n",
                            cfg.wifi_list[i].ssid, ip4addr_ntoa(&netif->ip_addr));
                    }
                }
            }

            if (!connected) {
                printf("NetworkManager: failed to connect to any configured network\n");
                ...
                if(scan_has_completed) {
                    ...
                    while (!connected && current!= NULL) {
                        ...
                        if (cyw43_arch_wifi_connect_timeout_ms(current->ssid, NULL, CYW43_AUTH_OPEN, 10000) == 0) {
                            connected = true;
                            struct netif* netif = netif_list;
                            ip_addr_t ip = netif->ip_addr;
                            ...
                            evt->type = EVENT_WIFI_CONNECTED;  
                            memset(evt->ssid, 0, 33); // Clear SSID
                            strncpy(evt->ssid, current->ssid, strlen(current->ssid));
                            memset(evt->ip, 0, 16); // Clear IP
                            ip4addr_ntoa_r(&ip, evt->ip, 15); // Convert IP to string
                            ...
                            printf("[EVENT] Connected to open network %s | IP: %s\n", current->ssid, ip4addr_ntoa(&ip));
            ...
            int curr_status = cyw43_wifi_link_status(&cyw43_state, CYW43_ITF_STA);

            if (curr_status != CYW43_LINK_JOIN || !check_connectivity_via_ping_gateway()) {
                ...
                event_t* event = create_event(EVENT_TYPE_NOTIFICATION, EVENT_SUBTYPE_NETWORK_DOWN, evt, sizeof(network_event_t));
