...
void pico_touch_screen_task(void *pvParameters) {
    ...
    static screen_control_t sScreen = {.mCursorX = 0,
                                       .mCursorY = 0,
                                       .mCursorType = 0,
                                       .mCanvasPaper = kBlack,
                                       .mCanvasInk = kWhite};
    ...
    (pin 12)  DC/RS      LCD register / data selection signal; high level:
    register. (pin 10)  SDI(MOSI)  SPI bus write data signal. (pin 09)  SCK SPI
    bus clock signal. (pin 36)  LED        Backlight control. (pin 06) SDO(MISO)
    SPI bus read data signal. Hasn't been used so far in here.
    ...
    */
    ...
    ILI9341_Init(sScreen.mpHWConfig, spi0, 30 * MHz, 4, 5, 6, 7, 8, 9);
    ...
    TouchInitHW(&touch_hwc, spi1, 2 * MHz, 12, 13, 10, 11, 15);
    ...
    TftPrintf(&sScreen, 0, 8, 0, 7,
              "Calibration mat:\n %.2f %.2f %.2f\n%.2f %.2f %.2f\n", cmat.KX1,
              cmat.KX2, cmat.KX3, cmat.KY1, cmat.KY2, cmat.KY3);
    ...
    for (;;) {
        ...
        if (touch_config.mIsProcessed) {
            ...
            if (0 == led_state % 128)
                TftPrintf(&sScreen, 0, 8, ~(touch_tick & 7), touch_tick & 7,
                          "%u %d %d %d %d\n", touch_tick, touch_config.mX,
                          touch_config.mY, x, y);
