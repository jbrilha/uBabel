        ...
        SLIST_FOREACH(chr, &svc->chrs, next) {
            if (!chr_is_empty(svc, chr) && SLIST_EMPTY(&chr->dscs) &&
                ...
                rc = ble_gattc_disc_all_dscs(
                    peer->conn_handle, chr->chr.val_handle,
    ...
    SLIST_FOREACH(svc, &peer->svcs, next) {
        if (!peer_svc_is_empty(svc) && SLIST_EMPTY(&svc->chrs)) {
            ...
            rc = ble_gattc_disc_all_chrs(
                peer->conn_handle, svc->svc.start_handle, svc->svc.end_handle,
...
static int peer_inc_add(struct peer *peer, uint16_t svc_start_handle,
    ...
    incl_svc =
        peer_incl_svc_find(cur_svc, gatt_incl_svc->handle, &incl_svc_prev);
    if (incl_svc != NULL) {
        /* Already discovered */
        return 0;
    }
...
static void peer_disc_incs(struct peer *peer) {
    ...
    if (svc != NULL && !peer_svc_is_empty(svc)) {
        rc =
            ble_gattc_find_inc_svcs(peer->conn_handle, svc->svc.start_handle,
