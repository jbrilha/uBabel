...
static const char *TAG = "COEXISTENCE";
...
example_ble_mesh_provisioning_cb(esp_ble_mesh_prov_cb_event_t event,
    ...
    case ESP_BLE_MESH_NODE_PROV_COMPLETE_EVT:
        ...
        node_prov_complete(param->node_prov_complete.net_idx,
    ...
    case ESP_BLE_MESH_SET_FAST_PROV_INFO_COMP_EVT: {
        ...
        ESP_LOGI(TAG,
                 "status_unicast: 0x%02x, status_net_idx: 0x%02x, status_match "
                 "0x%02x",
        ...
        err = example_handle_fast_prov_info_set_comp_evt(
            fast_prov_server.model,
            param->set_fast_prov_info_comp.status_unicast,
            param->set_fast_prov_info_comp.status_net_idx,
            param->set_fast_prov_info_comp.status_match);
        if (err != ESP_OK) {
            ESP_LOGE(TAG,
                     "%s: Failed to handle Fast Prov Info Set complete event",
                     __func__);
            return;
        }
        break;
    ...
    case ESP_BLE_MESH_SET_FAST_PROV_ACTION_COMP_EVT:
        ESP_LOGI(
            TAG,
            "ESP_BLE_MESH_SET_FAST_PROV_ACTION_COMP_EVT, status_action 0x%02x",
            param->set_fast_prov_action_comp.status_action);
        err = example_handle_fast_prov_action_set_comp_evt(
            fast_prov_server.model,
            param->set_fast_prov_action_comp.status_action);
        if (err != ESP_OK) {
            ESP_LOGE(TAG,
                     "%s: Failed to handle Fast Prov Action Set complete event",
                     __func__);
            return;
        }
        break;
...
static void
example_ble_mesh_custom_model_cb(esp_ble_mesh_model_cb_event_t event,
    ...
    case ESP_BLE_MESH_MODEL_OPERATION_EVT: {
        if (!param->model_operation.model ||
            !param->model_operation.model->op || !param->model_operation.ctx) {
        ...
        case ESP_BLE_MESH_VND_MODEL_OP_FAST_PROV_NODE_GROUP_DELETE: {
            ESP_LOGI(TAG,
                     "%s: Fast prov server receives msg, opcode 0x%04" PRIx32,
                     __func__, opcode);
            struct net_buf_simple buf = {
                .data = param->model_operation.msg,
                .len = param->model_operation.length,
            };
            err = example_fast_prov_server_recv_msg(
                param->model_operation.model, param->model_operation.ctx, &buf);
            if (err != ESP_OK) {
                ESP_LOGE(TAG, "%s: Failed to handle fast prov client message",
                         __func__);
                return;
            }
            break;
        ...
        case ESP_BLE_MESH_VND_MODEL_OP_FAST_PROV_NODE_ADDR_ACK: {
            ESP_LOGI(TAG,
                     "%s: Fast prov client receives msg, opcode 0x%04" PRIx32,
                     __func__, opcode);
            err = example_fast_prov_client_recv_status(
                param->model_operation.model, param->model_operation.ctx,
                param->model_operation.length, param->model_operation.msg);
            if (err != ESP_OK) {
                ESP_LOGE(TAG, "%s: Failed to handle fast prov server message",
                         __func__);
                return;
            }
            break;
        ...
        default:
            ESP_LOGI(TAG, "%s: opcode 0x%04" PRIx32, __func__,
    ...
    case ESP_BLE_MESH_MODEL_SEND_COMP_EVT:
        ...
        case ESP_BLE_MESH_VND_MODEL_OP_FAST_PROV_NODE_ADDR_STATUS:
            err = example_handle_fast_prov_status_send_comp_evt(
                param->model_send_comp.err_code, param->model_send_comp.opcode,
                param->model_send_comp.model, param->model_send_comp.ctx);
            if (err != ESP_OK) {
                ESP_LOGE(
                    TAG,
                    "%s: Failed to handle fast prov status send complete event",
                    __func__);
                return;
            }
            break;
    ...
    case ESP_BLE_MESH_CLIENT_MODEL_RECV_PUBLISH_MSG_EVT:
        ESP_LOGI(TAG,
                 "ESP_BLE_MESH_MODEL_CLIENT_RECV_PUBLISH_MSG_EVT, opcode "
                 "0x%04" PRIx32,
    ...
    case ESP_BLE_MESH_CLIENT_MODEL_SEND_TIMEOUT_EVT:
        ESP_LOGI(
            TAG,
            "ESP_BLE_MESH_CLIENT_MODEL_SEND_TIMEOUT_EVT, opcode 0x%04" PRIx32
            ", dst 0x%04x",
            param->client_send_timeout.opcode,
            param->client_send_timeout.ctx->addr);
        err = example_fast_prov_client_recv_timeout(
            param->client_send_timeout.opcode, param->client_send_timeout.model,
            param->client_send_timeout.ctx);
        if (err != ESP_OK) {
            ESP_LOGE(TAG, "%s: Failed to resend fast prov client message",
                     __func__);
            return;
        }
        break;
...
static void
example_ble_mesh_config_client_cb(esp_ble_mesh_cfg_client_cb_event_t event,
    ...
    if (param->error_code) {
        ESP_LOGE(TAG,
                 "Failed to send config client message, opcode: 0x%04" PRIx32,
                 opcode);
        return;
    }
    ...
    case ESP_BLE_MESH_CFG_CLIENT_SET_STATE_EVT:
        ...
        case ESP_BLE_MESH_MODEL_OP_APP_KEY_ADD: {
            ...
            if (node->reprov == false) {
                /* After sending Config AppKey Add successfully, start to send
                 * Fast Prov Info Set */
                if (fast_prov_server.unicast_cur >=
                    fast_prov_server.unicast_max) {
                    /* TODO:
                     * 1. If unicast_cur is >= unicast_max, we can also send the
                     * message to enable the Provisioner functionality on the
                     * node, and need to add another vendor message used by the
                     * node to require a new unicast address range from primary
                     * Provisioner, and before get the correct response, the
                     * node should pend the fast provisioning functionality.
                     * 2. Currently if address is not enough, the Provisioner
                     * will only add the group address to the node.
                     */
                    ESP_LOGW(TAG, "%s: Not enough address to be assigned",
                             __func__);
                    ...
                    if (fast_prov_server.unicast_cur +
                            fast_prov_server.unicast_step >=
                        fast_prov_server.unicast_max) {
                        node->unicast_max = fast_prov_server.unicast_max;
                    } else {
                        node->unicast_max = fast_prov_server.unicast_cur +
                                            fast_prov_server.unicast_step;
                    }
                    ...
                    memcpy(node->match_val, fast_prov_server.match_val,
                           fast_prov_server.match_len);
            ...
            if (node->lack_of_addr == false) {
                ...
                memcpy(&set.unicast_min, &node->unicast_min,
                       sizeof(example_node_info_t) -
                           offsetof(example_node_info_t, unicast_min));
            } else {
                set.ctx_flags = BIT(6);
            ...
            err = example_send_fast_prov_info_set(fast_prov_client.model, &info,
                                                  &set);
            if (err != ESP_OK) {
                ESP_LOGE(TAG, "%s: Failed to send Fast Prov Info Set message",
                         __func__);
                return;
            }
            break;
    ...
    case ESP_BLE_MESH_CFG_CLIENT_TIMEOUT_EVT:
        ...
        case ESP_BLE_MESH_MODEL_OP_APP_KEY_ADD: {
            ...
            err = example_send_config_appkey_add(config_client.model, &info,
                                                 NULL);
            if (err != ESP_OK) {
                ESP_LOGE(TAG, "%s: Failed to send Config AppKey Add message",
                         __func__);
                return;
            }
            break;
...
example_ble_mesh_config_server_cb(esp_ble_mesh_cfg_server_cb_event_t event,
    ...
    case ESP_BLE_MESH_CFG_SERVER_STATE_CHANGE_EVT:
        ...
        case ESP_BLE_MESH_MODEL_OP_APP_KEY_ADD:
            ...
            err = example_handle_config_app_key_add_evt(
                param->value.state_change.appkey_add.app_idx);
            if (err != ESP_OK) {
                ESP_LOGE(
                    TAG,
                    "%s: Failed to bind app_idx 0x%04x with non-config models",
...
static esp_err_t ble_mesh_init(void) {
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "%s: Failed to initialize fast prov server model",
                 __func__);
        return err;
    }
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "%s: Failed to initialize fast prov client model",
                 __func__);
        return err;
    }

    k_delayed_work_init(&send_self_prov_node_addr_timer,
